...
export default function PaymentModal({ isOpen, onClose, onSuccess }: PaymentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Actualizar estado de usuario a premium
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-[#0f2430] rounded-xl shadow-xl max-w-md w-full border border-slate-700">
        <div className="p-6">
          <h2 className="text-2xl font-bold text-white mb-6">Actualizar a Premium</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 rounded bg-slate-800/50 border border-slate-700 text-white placeholder-slate-500"
            ...
            <div className="pt-4">
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 rounded-lg bg-gradient-to-r from-[#5b34ff] to-[#b144ff] text-white font-semibold disabled:opacity-50"
              >
              ...
              <button
                type="button"
                onClick={onClose}
                className="w-full mt-2 py-2 text-slate-400 hover:text-white"
              >
                Cancelar
              </button>
            </div>
